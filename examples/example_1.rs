        ...
        Ok(replay) => {
            ...
            if let Some(ref life_bar) = replay.life_bar_graph {
                ...
                if !life_bar.is_empty() {
                    println!("First state: time={}ms, life={}", life_bar[0].time, life_bar[0].life);
                    println!("Last state: time={}ms, life={}", 
            ...
            if !replay.replay_data.is_empty() {
                ...
                for (i, event) in replay.replay_data.iter().take(5).enumerate() {
                    match event {
                        ReplayEvent::Osu(e) => {
                            println!("  {}: Osu - time_delta={}ms, x={}, y={}, keys={}", 
                                i + 1, e.time_delta, e.x, e.y, e.keys.value());
                        }
                        ReplayEvent::Taiko(e) => {
                            println!("  {}: Taiko - time_delta={}ms, x={}, keys={}", 
                                i + 1, e.time_delta, e.x, e.keys.value());
                        }
                        ReplayEvent::Catch(e) => {
                            println!("  {}: Catch - time_delta={}ms, x={}, dashing={}", 
                                i + 1, e.time_delta, e.x, e.dashing);
                        }
                        ReplayEvent::Mania(e) => {
                            println!("  {}: Mania - time_delta={}ms, keys={}", 
                                i + 1, e.time_delta, e.keys.value());
                        }
                    }
                }
            ...
            let total_time: i32 = replay.replay_data.iter()
            ...
            if total_time > 0 {
                ...
                println!("\nTotal replay duration: {}:{:02}.{:03}", minutes, seconds, milliseconds);
            }
